...
> Source: https://transformer-circuits.pub/2025/linebreaks/index.html
> literature/README.md line 1
> Format: markdown (converted from HTML); images in `images/when-models-manipulate-manifolds-counting/`
...
[Representing Character Count](#char-count)

[Sensing the Line Boundary](#boundary)

[Predicting the Newline](#prediction)

[A Distributed Character Counting Algorithm](#count-algo)
...
![](images/when-models-manipulate-manifolds-counting/fig-001.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-002.png)
...
Claude 3.5 Haiku is able to adapt to the line length for every value of k, predicting newlines at the correct positions with high probability by the third line (see [Appendix](#appendix-task-performance)).
...
1. Sparse crosscoder features.Each feature has an encoder, which acts as a linear + (Jump)ReLU probe on the residual stream, and a decoder. Ten features f\_1,\ldots,f\_{10} are associated with line character count. The model's estimate of the character count, given a residual stream vector x, is summarized by the set of activities of each of the 10 features \{f\_i(x)\}.
...
![](images/when-models-manipulate-manifolds-counting/fig-004.png)
...
In the [Appendix](#appendix-feature-splitting), we show these features are universal across dictionaries of different sizes, but that some feature splitting occurs with respect to the line width constraint.
...
![](images/when-models-manipulate-manifolds-counting/fig-005.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-006.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-007.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-008.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-009.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-010.png)
...
We find that newline tokens have their own dedicated character [counting features](#appendix-width-features) that activate based on the width of the line, counting the number of characters between adjacent newlines.
...
![](images/when-models-manipulate-manifolds-counting/fig-011.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-012.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-013.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-014.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-015.png)
...
#### [The Role of the Extra Dimensions](#one-dim)
...
![](images/when-models-manipulate-manifolds-counting/fig-016.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-017.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-018.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-019.png)

PCA of embedding vectors in W\_E averaged by token character length.

As with all of the counting manifolds, we also find [features](#appendix-token-lengths) that discretize this space into overlapping notions of short, medium, and long words.
...
![](images/when-models-manipulate-manifolds-counting/fig-020.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-021.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-022.png)
...
OV Circuit. The OV circuit coordinates with the QK circuit to create a heuristic estimate based on the number of tokens in the line multiplied by the average token length (\mu\_c \approx 4), with an additional length correction term. When attending to the newline, each head upweights the average token length multiplied by the head’s sink size: s\_h\times\mu\_c characters. If no attention is paid to the newline, then from the perspective of the head, the current token must be at least s\_h+r\_h tokens into the line and should upweight (s\_h+r\_h) \times \mu\_c character outputs. Finally, the OV circuit applies an additional correction depending on whether the tokens in the receptive field are above or below average in length.

Below, we include a detailed walkthrough of L0H1.

![](images/when-models-manipulate-manifolds-counting/fig-023.png)
...
For a more detailed analysis of each head, see [The Mechanics of Head Specialization](#appendix-mechanics). Layer 1 attention heads perform a similar operation, but additionally leverage the initial estimate of the character count (see [Layer 1 Head OVs](#appendix-l1-attn)).
...
![](images/when-models-manipulate-manifolds-counting/fig-024.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-025.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-026.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-027.png)
...
![](images/when-models-manipulate-manifolds-counting/fig-028.png)
